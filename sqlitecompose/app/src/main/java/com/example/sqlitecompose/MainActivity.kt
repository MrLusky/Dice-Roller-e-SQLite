    ...
    class DBHelper(context: Context) :
        ...
        fun getAllNotes(): List<Note> {
            ...
            val c: Cursor = readableDatabase.rawQuery(
                "SELECT id, title, content, date FROM notes ORDER BY id DESC",
                null
            )
            c.use { cur ->
                ...
                Row {
                    Button(
                        onClick = {
                            ...
                            if (editingId == null) {
                                // CREATE
                                dbHelper.insertNote(Note(title = t, content = c, date = d))
                            } else {
                                // UPDATE
                                dbHelper.updateNote(
                                    Note(id = editingId, title = t, content = c, date = d)
                                )
                            }
